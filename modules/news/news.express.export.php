		...
		foreach ($this->fields as $k => $field)
		{
			switch($field["type"])
			{
				...
				case 'images':
					...
					{
						if($images_variations = unserialize($this->diafan->configmodules("images_variations_".$this->export["element_type"], 'news', $this->export["site_id"])))
						{
					...
					foreach($images as $i)
					{
						...
						if(! empty($i["name"]))
						{
							$i["name"] = BASE_PATH.USERFILES.'/news/'.$this->cache['images_variation_large'].'/'.($i["folder_num"] ? $i["folder_num"].'/' : '').$i["name"];
						}
