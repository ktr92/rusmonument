...
class Update_admin_action extends Action_admin
{
	...
	private function download()
	{
		...
		if($id)
		{
			...
			if($hash) File::copy_file('http'.(IS_HTTPS ? "s" : '').'://user.diafan.ru/file/update/'.$id.'/'.$hash, 'return/'.$id.'.zip');
			// else File::copy_file('http'.(IS_HTTPS ? "s" : '').'://user.diafan.ru/api/file/get/update/'.$id.'.zip'.'?'.$this->diafan->uid(), 'return/'.$id.'.zip');
			else File::save_file(file_get_contents('http'.(IS_HTTPS ? "s" : '').'://user.diafan.ru/api/file/get/update/'.$id.'.zip'.'?'.$this->diafan->uid()), 'return/'.$id.'.zip');
