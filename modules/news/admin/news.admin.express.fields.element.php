...
/**
 * News_admin_express_fields_element
 */
class News_admin_express_fields_element extends Frame_admin_express_fields_element
{
	/**
	...
	public $variables = array (
		'main' => array (
			...
			'type' => array(
				'type' => 'select',
				'name' => 'Тип',
				'help' => 'Значение или свойство новости, куда будет импортироваться данное поле.',
				'select' => array(
					...
					'cats' => 'Категория', // идентификатор категории из файла импорта категорий, используется у новостей. Чтобы категория для новости определилась правильно, нужно сначала импортировать категории
					...
					'prior' => 'Маркер для новости (всегда сверху)', // поле «Важно (всегда сверху)»
					...
					'rel_news' => 'ID связанных новостей', // идентификаторы через «Разделитель данных внутри поля», только для новостей
					...
					'rewrite' => 'Псевдоссылка', // ЧПУ новости/категории
					...
					'sort' => 'Номер для сортировки', // новости сортируются по убыванию, категории по возрастанию
					...
					'view_rows' => 'Шаблон модуля для списка новостей (modules/news/views/news.view.шаблон.php)', // (modules/news/views/news.view.шаблон.php)
			...
			'params_id' => array(
				...
				'help' => "Поле выводится только для типов «Идентификатор», «Категория», «Родитель» и «Идентификатор связанных новостей».\n\n* собственное значение – при первом импорте все новости/категории добавляться в базу, идентификатор запишется в поле import_id. При последующем импорте новости/категории будут обновляться по идентификатору import_id;\n* идентификатор на сайте – использовать стандартный идентификатор id;\n* артикул – только для новостей, только для типов «Идентификатор» и «Идентификатор связанных новостей»;\n* название – только для категорий, только для типов «Категория» и «Родитель».",
				'no_save' => true,
			),
	...
	public function edit_variable_params()
	{
		...
		// id,  rel_news
		$param_type = (in_array($type, array('id', 'rel_news')) && ! empty($params["type"]) ? $params["type"] : '');
		echo '
		<div id="param_type" class="unit params param_id param_rel_news">
	...
	public function table_variable_params($id = 0)
	{
		...
		$param_type = (in_array($type, array('id', 'rel_news')) && ! empty($params["type"]) ? $params["type"] : '');
		echo '
		<div unit_id="param_type" class="unit params param_id param_rel_news">
			<div class="infofield">'.$this->diafan->_('В этом поле загружается').'</div>
			<select name="param_type[]">
			<option value=""'.(! $param_type ? ' selected' : '').'>'.$this->diafan->_('Импортируемый ID').'</option>
			<option value="site"'.($param_type == 'site' ? ' selected' : '').'>'.$this->diafan->_('ID на сайте').'</option>
