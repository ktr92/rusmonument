		...
		foreach ($this->fields as $k => $field)
		{
			switch($field["type"])
			{
				...
				case 'images':
					...
					if(! isset($this->cache['images_variation_large']))
					{
						if($images_variations = unserialize($this->diafan->configmodules("images_variations_".$this->export["element_type"], 'clauses', $this->export["site_id"])))
						{
							foreach($images_variations as $images_variation)
							{
								if($images_variation["name"] == 'large')
								{
									$this->cache['images_variation_large'] = DB::query_result("SELECT folder FROM {images_variations} WHERE id=%d LIMIT 1", $images_variation["id"]);
									continue;
								}
							}
						}
						if(empty($this->cache['images_variation_large'])) $this->cache['images_variation_large'] = 'original';
					}
					foreach($images as $i)
					{
						...
						if(! empty($i["name"]))
						{
							$i["name"] = BASE_PATH.USERFILES.'/clauses/'.$this->cache['images_variation_large'].'/'.($i["folder_num"] ? $i["folder_num"].'/' : '').$i["name"];
						}
